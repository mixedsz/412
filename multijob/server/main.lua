...
AddEventHandler("core_multijob:addJob", function(job)
    local src = source
    print("[JOB SELECTOR SERVER] Add job event received from player", src)
    print("[JOB SELECTOR SERVER] Job to add:", job)
    local xPlayer = ESX.GetPlayerFromId(src)
    if not xPlayer then
        print("[JOB SELECTOR SERVER] ERROR: xPlayer not found")
        return
    end
    print("[JOB SELECTOR SERVER] Player identifier:", xPlayer.identifier)
    AddJob(xPlayer.identifier, job, 0, true)
    print("[JOB SELECTOR SERVER] Job added successfully")
...
AddEventHandler("core_multijob:changeJob", function(job, grade)
    local src = source
    print("[JOB SELECTOR SERVER] Change job event received from player", src)
    print("[JOB SELECTOR SERVER] Job:", job, "Grade:", grade)
    local xPlayer = ESX.GetPlayerFromId(src)
    if not xPlayer then
        print("[JOB SELECTOR SERVER] ERROR: xPlayer not found")
        return
    ...
    print("[JOB SELECTOR SERVER] Player identifier:", xPlayer.identifier)
    print("[JOB SELECTOR SERVER] Current job:", xPlayer.job.name, "Grade:", xPlayer.job.grade)

    local canAssignJob = false
    MySQL.Async.fetchAll("SELECT * FROM user_jobs WHERE identifier = @identifier", {["@identifier"] = xPlayer.identifier}, function(jobs)
        print("[JOB SELECTOR SERVER] Fetched", #jobs, "jobs from database")
        
        for k, v in pairs(Config.DefaultJobs) do
            if v.job == job then
                canAssignJob = true
                print("[JOB SELECTOR SERVER] Job", job, "is a default job")
                break
            end
        end
        ...
        if not canAssignJob then
            if job == 'offduty' or job == 'unemployed' or job == Config.DefaultJobWhenPlayerIsFire then
                canAssignJob = true
                print("[JOB SELECTOR SERVER] Job", job, "is a system job")
            elseif jobs and #jobs > 0 then
                for _, v in ipairs(jobs) do
                    if v.job == job then
                        canAssignJob = true
                        print("[JOB SELECTOR SERVER] Job", job, "found in player's jobs")
                        break
                    end
                end
            end
        ...
        if not canAssignJob then
            print("[JOB SELECTOR SERVER] ERROR: Player not allowed to have job", job)
            xPlayer.kick('[KICKED] Not allowed, sorry.')
            return
        end
        
        print("[JOB SELECTOR SERVER] Setting job to:", job, "Grade:", grade)
        xPlayer.setJob(job, grade)
        print("[JOB SELECTOR SERVER] Job changed successfully")
    end)        
end)
...
AddEventHandler("core_multijob:removeJob", function(job, grade)
    local src = source
    print("[JOB SELECTOR SERVER] Remove job event received from player", src)
    print("[JOB SELECTOR SERVER] Job to remove:", job, "Grade:", grade)
    local xPlayer = ESX.GetPlayerFromId(src)
    if not xPlayer then
        print("[JOB SELECTOR SERVER] ERROR: xPlayer not found")
        return
    ...
    print("[JOB SELECTOR SERVER] Job removed successfully")
end)